...
# Dockerfile
LOGROUTER_DOCKERFILE_NAME="Dockerfile.logrouter.1.1"


# バージョン情報
LOGROUTER_IMAGE_VERSION="1.1"

# ECRリポジトリのURL (AWSアカウントIDは.envファイルから取得)
LOGROUTER_REPOSITORY_URL="${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/econas-prod2-log-router-repo:${LOGROUTER_IMAGE_VERSION}"
...
# ビルド実行
echo "🔨 Building log-router image..."
...
docker buildx build \
    ...
    -t econas-log-router:${LOGROUTER_IMAGE_VERSION}-x86_64 \
    -f ${LOGROUTER_DOCKERFILE_NAME} \
    .
...
if [ $build_result -eq 0 ]; then
    ...
    docker tag econas-log-router:${LOGROUTER_IMAGE_VERSION}-x86_64 ${LOGROUTER_REPOSITORY_URL}
    ...
    echo ""
    echo "docker push ${LOGROUTER_REPOSITORY_URL}"
    echo ""
else
