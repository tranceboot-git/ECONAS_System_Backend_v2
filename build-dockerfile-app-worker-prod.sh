...
# ------------------------------------------------------------------------------------------------
#
# ECONAS App Workerコンテナ
# Productionイメージ Build Script
#
# ------------------------------------------------------------------------------------------------


echo "🚀 Starting ECONAS app worker production build..."
...
# バージョン情報
APP_WORKER_INIT_IMAGE_VERSION="1.1"
APP_WORKER_IMAGE_VERSION="1.1"

# ECRリポジトリのURL (AWSアカウントIDは.envファイルから取得)
APP_WORKER_INIT_REPOSITORY_URL="${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/econas-prod2-worker-init-repo:${APP_WORKER_INIT_IMAGE_VERSION}"
APP_WORKER_REPOSITORY_URL="${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/econas-prod2-worker-repo:${APP_WORKER_IMAGE_VERSION}"
...
# ビルド実行
echo "🔨 Building app worker image..."
...
docker buildx build \
    ...
    -t econas-app-worker-init:${APP_WORKER_INIT_IMAGE_VERSION}-x86_64 \
...
docker buildx build \
    ...
    --build-arg EFS_APPLICATION_MOUNT_PATH=/efs \
    --build-arg EPHEMERAL_STORAGE_TMP_PATH=/tmp \
    -t econas-app-worker:${APP_WORKER_IMAGE_VERSION}-x86_64 \
    -f ${APP_DOCKERFILE_NAME} \
    .
...
if [ $build_result -eq 0 ]; then
    ...
    docker tag econas-app-worker-init:${APP_WORKER_INIT_IMAGE_VERSION}-x86_64 ${APP_WORKER_INIT_REPOSITORY_URL}
    docker tag econas-app-worker:${APP_WORKER_IMAGE_VERSION}-x86_64 ${APP_WORKER_REPOSITORY_URL}
    ...
    echo ""
    echo "docker push ${APP_WORKER_INIT_REPOSITORY_URL}"
    echo ""
    echo "docker push ${APP_WORKER_REPOSITORY_URL}"
    echo ""
else
